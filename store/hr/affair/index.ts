...
export const affairStore = defineStore("affairStore", {
  ...
  actions: {
    //부서에 소속된 사원 조회
    async FETCH_EMP_DEPT_IN(deptCode: string) {
      // 객체로 넘어오면 {}
      try {
     ...
     * 직원 정보 수정
     */
    async UPDATE_EMP_INFO(empUpdate: any) {
      // 객체로 넘어오면 {}
     ...
     * 직원 사진 수정
     */
    async UPDATE_PHOTO_FILE(empPhoto: any) {
      // 객체로 넘어오면 {}
     ...
     * 직원 삭제
     */
    async DELETE_SELECTED_EMP(emp: any) {
      // 객체로 넘어오면 {}
      console.log(emp);
      try {
     ...
     * 발령 등록
     */
    async INSERT_APPOINT_EMP(empApp: any) {
      // 객체로 넘어오면 {}
